...
class JointFabricPkiCluster(private val controller: MatterController, private val endpointId: UShort) {
  class SignNOCIssuerRequest(
    val NOCIssuerCSR: ByteArray
  )

  class JointFabricResponse(
...
class AttributeListAttribute(
  ...
  suspend fun jointFabricRequest(fabricIndex: ULong
      ,timedInvokeTimeout: Duration? = null): SignNOCIssuerRequest {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_N_O_C_ISSUER_C_S_R)) {NOCIssuerCSR_decoded = tlvReader.getByteArray(tag)}  
    ...
    return SignNOCIssuerRequest(
      NOCIssuerCSR_decoded
    )
  }

  suspend fun signNOCIssuerResponse(statusCode: UByte
      ,NOCIssuerCert: ByteArray
      ,nodeId: ULong
      ,fabricId: ULong
      ,adminVendorId: UShort
      ,caseAdminSubject: ULong
      ,timedInvokeTimeout: Duration? = null): JointFabricResponse {
