...
CHIP_ERROR ExampleOperationalCredentialsIssuer::SignICACAfterValidation(const ByteSpan & icaCsr, MutableByteSpan & icac)
{
...
CHIP_ERROR ExampleOperationalCredentialsIssuer::SignICAC(const ByteSpan & icaCsr, Callback::Callback<OnICACSigned> * onCompletion)
{
    VerifyOrReturnError(mInitialized, CHIP_ERROR_UNINITIALIZED);
    ...
    ReturnErrorOnFailure(SignICACAfterValidation(icaCsr, icacSpan));
